...
static void do_say(struct client *p, char *s);
...
int main(int argc, char **argv)
{
	...
	/*set up listening socket for new connections*/
	...
	while(1) {
		...
		default:
			...
			/*do stuff with an aleady conected client*/
			else {
				for(p = clientlist; p; p = p-> next) {
					if(FD_ISSET(p->fd, &fdlist)) {
						/*if we overflow 200, reset*/
						if(p->bytes_in_buf >= 200) {
							p->bytes_in_buf = 0;
							memset(&(p->buf),'\0',sizeof p->buf);
						}
						
						if((read_len = read(p->fd,p->buf,200 - p->bytes_in_buf)) < 0) {
							perror("read");
							return(1);
						}
						
						/*if we hit end of file, remove the client*/
						if(read_len == 0)
							removeclient(p);

						p->bytes_in_buf = p->bytes_in_buf + read_len;

						/*do reading operations*/
						if((s = memnewline(p->buf,p->bytes_in_buf)) != NULL)
							do_something(p,s);
						break;
					}
				}
