...
static void do_something()
{
	...
	if (select(serverfd+1, &fdlist, NULL, NULL, NULL) < 0) {
		...
		if (fgets(buf, sizeof buf, stdin) == NULL)
		    exit(0);
		if(strncmp(buf, lang_say, strlen(lang_say)) == 0 && buf[strlen(lang_say)] == ' ') {
			sprintf(sendbuf, "say %s\r\n", buf + strlen(lang_say) + 1);
			send_string(sendbuf);	
		} else {
		docmd(explode(buf));
		}
...
static void do_something_server(char *wherenewline)
{
    ...
    *wherenewline = '\0';
	...
	printf("%s\n", lang_get_nosuch);
	} else if (strncmp(buf, "said ", 5) == 0) {
	char *end; 
	int id;
	id = strtol(buf + 5, &end, 10);
